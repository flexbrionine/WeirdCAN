 ...
 * Print registers.
*/ 
void WeirdCAN::printRegister(const char * buf, uint32_t reg) {
  if (DEBUG == 0) return;
  Serial.print(buf);
 ...
 *
 */
void WeirdCAN::CANSetGpio(GPIO_TypeDef * addr, uint8_t index, uint8_t speed = 3) {
    ...
    uint32_t mask;
    WeirdCAN::printRegister("GPIO_AFR(b)=", addr->AFR[1]);
    mask = 0xF << _index4;
    addr->AFR[ofs]  &= ~mask;         // Reset alternate function
    setting = 0x9;                    // AF9
    mask = setting << _index4;
    addr->AFR[ofs]  |= mask;          // Set alternate function
    WeirdCAN::printRegister("GPIO_AFR(a)=", addr->AFR[1]);

    WeirdCAN::printRegister("GPIO_MODER(b)=", addr->MODER);
    mask = 0x3 << _index2;
    addr->MODER   &= ~mask;           // Reset mode
    setting = 0x2;                    // Alternate function mode
    mask = setting << _index2;
    addr->MODER   |= mask;            // Set mode
    WeirdCAN::printRegister("GPIO_MODER(a)=", addr->MODER);

    WeirdCAN::printRegister("GPIO_OSPEEDR(b)=", addr->OSPEEDR);
    mask = 0x3 << _index2;
    addr->OSPEEDR &= ~mask;           // Reset speed
    setting = speed;
    mask = setting << _index2;
    addr->OSPEEDR |= mask;            // Set speed
    WeirdCAN::printRegister("GPIO_OSPEEDR(a)=", addr->OSPEEDR);

    WeirdCAN::printRegister("GPIO_OTYPER(b)=", addr->OTYPER);
    mask = 0x1 << index;
    addr->OTYPER  &= ~mask;           // Reset Output push-pull
    WeirdCAN::printRegister("GPIO_OTYPER(a)=", addr->OTYPER);

    WeirdCAN::printRegister("GPIO_PUPDR(b)=", addr->PUPDR);
    mask = 0x3 << _index2;
    addr->PUPDR   &= ~mask;           // Reset port pull-up/pull-down
    WeirdCAN::printRegister("GPIO_PUPDR(a)=", addr->PUPDR);
}
 ...
 *
 */
void WeirdCAN::CANSetFilter(uint8_t index, uint8_t scale, uint8_t mode, uint8_t fifo, uint32_t bank1, uint32_t bank2) {
  if (index > 27) return;
 ...
 *
 */
bool WeirdCAN::CANInit(BITRATE bitrate, int remap)
{
  // Reference manual
  ...
  if (remap == 0) {
    ...
    WeirdCAN::CANSetGpio(GPIOA, 11);               // Set PA11
    WeirdCAN::CANSetGpio(GPIOA, 12);               // Set PA12
    
    // CAN2
    RCC->AHB1ENR |= 0x2;                 // Enable GPIOB clock 
    WeirdCAN::CANSetGpio(GPIOB, 5);                // Set PB5
    WeirdCAN::CANSetGpio(GPIOB, 6);                // Set PB6
  }

  if (remap == 2) {
    // CAN1
    RCC->AHB1ENR |= 0x2;                 // Enable GPIOB clock 
    WeirdCAN::CANSetGpio(GPIOB, 8);                // Set PB8
    WeirdCAN::CANSetGpio(GPIOB, 9);                // Set PB9

    // CAN2
    RCC->AHB1ENR |= 0x2;                 // Enable GPIOB clock 
    WeirdCAN::CANSetGpio(GPIOB, 12);               // Set PB12
    WeirdCAN::CANSetGpio(GPIOB, 13);               // Set PB13
  }
    
  if (remap == 3) {
    // CAN1
    RCC->AHB1ENR |= 0x8;                 // Enable GPIOD clock 
    WeirdCAN::CANSetGpio(GPIOD, 0);                // Set PD0
    WeirdCAN::CANSetGpio(GPIOD, 1);                // Set PD1
  ...
  WeirdCAN::printRegister("CAN1->BTR=", CAN1->BTR);
  ...
  WeirdCAN::printRegister("CAN2->BTR=", CAN2->BTR);

  // Configure Filters to default values
  CAN1->FMR  |=   0x1UL;                 // Set to filter initialization mode
  CAN1->FMR  &= 0xFFFFC0FF;              // Clear CAN2 start bank
  WeirdCAN::printRegister("CAN1->FMR=", CAN1->FMR);
  ...
  WeirdCAN::CANSetFilter(0, 1, 0, 0, 0x0UL, 0x0UL); 
  ...
  // Filter assigned to FIFO 0 
  // Filter bank register to all 0
  WeirdCAN::CANSetFilter(14, 1, 0, 0, 0x0UL, 0x0UL); 
 ...
 * 
 */
void WeirdCAN::CANReceive(uint8_t ch, CAN_msg_t* CAN_rx_msg)
{
  if(ch == 1) {
 ...
 * 
 */
void WeirdCAN::CANSend(uint8_t ch, CAN_msg_t* CAN_tx_msg)
{
 ...
 *
 */
uint8_t WeirdCAN::CANMsgAvail(uint8_t ch)
{
  if (ch == 1) {
